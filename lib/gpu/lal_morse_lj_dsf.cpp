...
#include "morse_lj_dsf_cl.h"
#elif defined(USE_CUDART)
const char *morse_lj_dsf=0;
#else
#include "morse_lj_dsf_cubin.h"
#endif

#include "lal_morse_lj_dsf.h"
#include <cassert>
using namespace LAMMPS_AL;
#define MORLJDSFT MORLJDSF<numtyp, acctyp>

extern Device<PRECISION,ACC_PRECISION> device;

template <class numtyp, class acctyp>
MORLJDSFT::MORLJDSF() : BaseCharge<numtyp,acctyp>(),
                                    _allocated(false) {
}
...
template <class numtyp, class acctyp>
int MORLJDSFT::bytes_per_atom(const int max_nbors) const {
  return this->bytes_per_atom_atomic(max_nbors);
}

template <class numtyp, class acctyp>
int MORLJDSFT::init(const int ntypes, double **host_cutsq, double **host_lj1,
  ...
  success=this->init_atomic(nlocal,nall,max_nbors,maxspecial,cell_size,gpu_split,
                            _screen,morse_lj_dsf,"k_morse_lj_dsf");
  if (success!=0)
    return success;
...
template <class numtyp, class acctyp>
double MORLJDSFT::host_memory_usage() const {
  return this->host_memory_usage_atomic()+sizeof(MORLJDSF<numtyp,acctyp>);
}
...
// ---------------------------------------------------------------------------
template <class numtyp, class acctyp>
void MORLJDSFT::loop(const bool _eflag, const bool _vflag) {
...
template class MORLJDSF<PRECISION,ACC_PRECISION>;
