...
PairMorseLJCoulDSF::PairMorseLJCoulDSF(LAMMPS *lmp) : Pair(lmp)
{
  single_enable = 0;
}
...
PairMorseLJCoulDSF::~PairMorseLJCoulDSF()
{
  if (!copymode) {
    if (allocated) {
...
void PairMorseLJCoulDSF::compute(int eflag, int vflag)
{
...
void PairMorseLJCoulDSF::allocate()
{
  allocated = 1;
  int n = atom->ntypes;
...
void PairMorseLJCoulDSF::settings(int narg, char **arg)
{
...
void PairMorseLJCoulDSF::coeff(int narg, char **arg)
{
  if (narg < 7 || narg > 8)
  ...
  double epsilon_one = force->numeric(FLERR,arg[5]);
  double sigma_one = force->numeric(FLERR,arg[6]);
...
void PairMorseLJCoulDSF::init_style()
{
  if (!atom->q_flag)
...
double PairMorseLJCoulDSF::init_one(int i, int j)
{
  if (setflag[i][j] == 0) {
...
void PairMorseLJCoulDSF::write_restart(FILE *fp)
{
...
void PairMorseLJCoulDSF::read_restart(FILE *fp)
{
...
void PairMorseLJCoulDSF::write_restart_settings(FILE *fp)
{
...
void PairMorseLJCoulDSF::read_restart_settings(FILE *fp)
{
  if (comm->me == 0) {
...
double PairMorseLJCoulDSF::single(int i, int j, int itype, int jtype, double rsq,
...
void *PairMorseLJCoulDSF::extract(const char *str, int &dim)
{
  dim = 2;
